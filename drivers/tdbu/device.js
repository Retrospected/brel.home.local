...
class TDBUDevice extends Device {
   ...
   */
   async onInit() {
    ...
    this.registerCapabilityListener('windowcoverings_tilt_down', async (value) => {
      this.log('State set: ',value)
      this.deviceapi.windowcoverings_tilt_down(this.getData()['id'],this.getData()['deviceType'], value)
      this.paused = true;
      const waitFor = delay => new Promise(resolve => setTimeout(resolve, delay));
      await waitFor(10000);
      this.paused = false;
    ...
    this.registerCapabilityListener('windowcoverings_tilt_set', async (value) => {
      this.log('State set: ',value)
      this.deviceapi.windowcoverings_tilt_set(this.getData()['id'],this.getData()['deviceType'], value)
      this.paused = true;
      const waitFor = delay => new Promise(resolve => setTimeout(resolve, delay));
      await waitFor(10000);
      this.paused = false;
    ...
    this.registerCapabilityListener('windowcoverings_tilt_up', async (value) => {
      this.log('State set: ',value)
      this.deviceapi.windowcoverings_tilt_up(this.getData()['id'],this.getData()['deviceType'], value)
      this.paused = true;
      const waitFor = delay => new Promise(resolve => setTimeout(resolve, delay));
      await waitFor(10000);
      this.paused = false;
